...
test.describe("Student Login", () => {
  test("login page loads with correct elements", async ({ page }) => {
    await page.goto(URLS.studentLogin, { waitUntil: "networkidle" });
  ...
  test("shows heading and panel label", async ({ page }) => {
    ...
    await expect(page.getByText("Sporcu hesabiniza giris yapin")).toBeVisible();
  });
  ...
  test("shows error toast for invalid credentials", async ({ page }) => {
    ...
    await page.locator("#email").fill(CREDENTIALS.student.email);
  ...
  test("successful login redirects to student panel", async ({ page }) => {
    await page.goto(URLS.studentLogin, { waitUntil: "networkidle" });

    await page.locator("#email").fill(CREDENTIALS.student.email);
    await page.locator("#password").fill(CREDENTIALS.student.password);
    await page.getByRole("button", { name: /Giris Yap/i }).click();

    await expect(page).toHaveURL(/\/student\/student/, { timeout: 15000 });
  });

  test("protected student route redirects to student login", async ({
    page,
  }) => {
    await page.goto(URLS.studentDashboard);
